...
describe('AccountPaymentSplitter', () => {
  ...
  beforeEach(async () => {
    ...
    const AccountPaymentSplitter = await ethers.getContractFactory(
      'AccountPaymentSplitter'
    );
    const [owner, addr1, addr2] = await ethers.getSigners();
    contract = await upgrades.deployProxy(AccountPaymentSplitter, [
